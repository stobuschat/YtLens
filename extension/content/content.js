 ...
 * This file is par of YtLens - a browser extension for customizing YouTube's recommendations
 * 
 * Copyright 2025 Sebastian Tobuschat
 * 
...
class YouTubeContentFilter {
  ...
  SELECTORS = {
    ...
    CHANNEL:
      "#channel-name, .ytd-channel-name, .ytm-channel-name, .yt-core-attributed-string",
    // Specific channel name selectors (prioritized for accuracy)
    CHANNEL_DESKTOP: "#text.ytd-channel-name",
    CHANNEL_MOBILE:
      ".ytm-badge-and-byline-item-byline > .yt-core-attributed-string",
    ...
    MENU_BUTTON:
      "ytd-menu-renderer yt-icon-button, ytm-menu-renderer yt-icon-button",
      ...
      list.forEach((item, index) => {
        ...
        if (item.pattern && typeof item.pattern === "string") {
          try {
            ...
            this.log(
              `Invalid regex pattern in ${listName} item #${index} ('${item.name}'): ${item.pattern}`,
              e
            );
          ...
          item.keywordPatterns = item.keywords
            .map((keyword) => {
              ...
              try {
                ...
                this.log(
                  `Invalid keyword in ${listName} item #${index} ('${item.name}'): ${keyword}`,
                  e
                );
                return null;
              }
  ...
  _setupLanguagePatterns() {
    ...
    if (
      this.config.youtubeLanguage &&
      this.buttonPatterns[this.config.youtubeLanguage]
    ) {
      ...
      try {
        ...
        const ytLangMeta = document.querySelector(
          'meta[http-equiv="Content-Language"]'
        );
        const detectedLang = (htmlLang || ytLangMeta?.content || "")
          .split("-")[0]
          .toLowerCase();
    ...
    this.activeNotInterestedPattern =
      this.buttonPatterns[activeLang]?.notInterested;
    this.activeStrictBlockingPattern =
      this.buttonPatterns[activeLang]?.dontRecommendChannel;

    // 3. Override with custom patterns if provided
    if (this.config.customNotInterestedPattern) {
      try {
        this.activeNotInterestedPattern = new RegExp(
          this.config.customNotInterestedPattern,
          "i"
        );
    ...
    if (this.config.customDontRecommendPattern) {
      try {
        this.activeStrictBlockingPattern = new RegExp(
          this.config.customDontRecommendPattern,
          "i"
        );
  ...
  extractVideoData(videoElement) {
    try {
      ...
      const descriptionElement = videoElement.querySelector(
  ...
  _checkFilterMatch(videoData, filter) {
    ...
    const checkField = (fieldValue, lowerFieldValue, fieldName, config) => {
      ...
      if (config.keywordPatterns) {
        const matchedPattern = config.keywordPatterns.find((pattern) =>
          pattern.test(lowerFieldValue)
        );
        if (matchedPattern) {
  ...
  filterVideos() {
    ...
    unprocessedVideos.forEach((videoContainer) => {
      // Mark as processed immediately to prevent re-processing in case of errors
      ...
      if (!videoData) {
        ...
        return; // Skip if data extraction failed
      }
    ...
    if (processedCount > 0) {
        this.debug(
            `Filtering pass complete: ${blockedCount}/${processedCount} new videos blocked (${(
                endTime - startTime
            ).toFixed(1)}ms)`
        );
    }
          ...
          const check = () => {
              ...
              if (menuItems.length > 0) {
                  ...
                  if (this.config.useStrictBlocking && this.activeStrictBlockingPattern) {
                      targetButton = Array.from(menuItems).find(el =>
                          this.activeStrictBlockingPattern.test(el.textContent)
                      );
                  }
                  ...
                  if (!targetButton && this.activeNotInterestedPattern) {
                      targetButton = Array.from(menuItems).find(el =>
                          this.activeNotInterestedPattern.test(el.textContent)
                      );
                  }
  ...
  startObserving() {
    ...
    this.debug(
      `Starting observer. Initial tab state: ${
        this.isTabActive ? "active" : "inactive"
      }`
    );
  ...
  handleVisibilityChange() {
    ...
    this.debug(
      `Tab visibility changed: ${wasActive ? "active" : "inactive"} -> ${
        this.isTabActive ? "active" : "inactive"
      }`
    );
...
if (document.readyState === "loading") {
  ...
  document.addEventListener("DOMContentLoaded", initYouTubeFilter, { once: true });
} else {
