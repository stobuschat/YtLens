...
document.addEventListener("DOMContentLoaded", () => {
  ...
  class OptionsManager {
    ...
    elements = {
      ...
      customNotInterestedInput: document.getElementById(
        "customNotInterestedPattern"
      ),
      customDontRecommendInput: document.getElementById(
      ...
      ruleChannelsInput: document.getElementById("rule-channels"), // Assuming this exists, add if needed
    ...
    _addEventListeners() {
      ...
      // Forms
      this.elements.optionsForm.addEventListener(
        "submit",
        this._handleOptionsFormSubmit
      );

      // Buttons
      this.elements.loadPresetButton.addEventListener("click", this._handleLoadPreset);
      this.elements.addBlacklistRuleBtn.addEventListener("click", () =>
        this.openRuleModal("blacklist")
      );
      this.elements.addWhitelistRuleBtn.addEventListener("click", () =>
        this.openRuleModal("whitelist")
      );
      this.elements.importBlacklistBtn.addEventListener("click", () =>
        this.importRules("blacklist")
      );
      this.elements.exportBlacklistBtn.addEventListener("click", () =>
        this.exportRules("blacklist")
      );
      this.elements.importWhitelistBtn.addEventListener("click", () =>
        this.importRules("whitelist")
      );
      this.elements.exportWhitelistBtn.addEventListener("click", () =>
        this.exportRules("whitelist")
      );
      ...
      // Language Settings
      this.elements.youtubeLanguageSelect.addEventListener(
        "change",
    ...
    _handleTabClick(event) {
      ...
      document
        .getElementById(`${clickedTab.dataset.tab}-tab`)
    ...
    _handleLanguageChange() {
      ...
      this.elements.customPatternsSection.style.display = isCustom
        ? "block"
        : "none";
    }
    ...
    async _loadOptions() {
      try {
        ...
        this.state.settings.customNotInterestedPattern =
          data.customNotInterestedPattern || "";
        this.state.settings.customDontRecommendPattern =
          data.customDontRecommendPattern || "";
    ...
    async _saveOptions() {
      const settingsToSave = {
        ...
        customNotInterestedPattern:
          this.elements.customNotInterestedInput.value.trim(),
        customDontRecommendPattern:
          this.elements.customDontRecommendInput.value.trim(),
      };
    ...
    _updateUIFromState() {
      ...
      this.elements.useStrictBlockingCheckbox.checked =
        this.state.settings.useStrictBlocking;

      // Update language settings
      this.elements.youtubeLanguageSelect.value =
        this.state.settings.youtubeLanguage;
      this.elements.customNotInterestedInput.value =
        this.state.settings.customNotInterestedPattern;
      this.elements.customDontRecommendInput.value =
        this.state.settings.customDontRecommendPattern;
    ...
    _renderRulesTable(listType) {
      const rules = listType === "blacklist"
          ? this.state.blacklistRules
          : this.state.whitelistRules;
      const tableBody = listType === "blacklist"
          ? this.elements.blacklistTableBody
          : this.elements.whitelistTableBody;
    ...
    _createRuleRow(listType, rule, index) {
      ...
      row.appendChild(
        createCheckboxCell(rule.onTitle, (e) => (rule.onTitle = e.target.checked))
      );
      row.appendChild(
        createCheckboxCell(
          rule.onDescription,
          (e) => (rule.onDescription = e.target.checked)
        )
      );
      row.appendChild(
        createCheckboxCell(
          rule.onChannelName,
          (e) => (rule.onChannelName = e.target.checked)
        )
      );
      ...
      if (listType === "blacklist") {
        row.appendChild(
          createCheckboxCell(
            ...
            (e) => (rule.markNotInterested = e.target.checked)
          )
        );
      }
    ...
    openRuleModal(listType, ruleIndex) {
      ...
      this.elements.modalTitle.textContent = isEditing
        ? "Edit Rule"
        : "Add New Rule";

      // Toggle visibility of "Mark Not Interested" option
      this.elements.notInterestedOptionDiv.style.display =
        listType === "blacklist" ? "inline-block" : "none";

      // Populate form if editing, otherwise reset
      if (isEditing) {
        const rule = listType === "blacklist"
            ? this.state.blacklistRules[ruleIndex]
            : this.state.whitelistRules[ruleIndex];
        ...
        this.elements.ruleChannelsInput.value = rule.channels?.join(", ") || ""; // Assuming rule.channels exists
    ...
    _saveRule() {
      ...
      const rule = {
        ...
        markNotInterested: listType === "blacklist"
            ? this.elements.ruleNotInterestedCheckbox.checked
            : false, // Not applicable for whitelist
      };
      ...
      input.onchange = (event) => {
        ...
        reader.onload = (e) => {
          try {
            ...
            this._renderRulesTable(listType);
            alert(`Successfully imported ${validatedRules.length} rules into ${listType}. Remember to save your options.`);
          } catch (error) {
